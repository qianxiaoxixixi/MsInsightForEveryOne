...
main() {
    ...
    start_time=$(date "+%s")
    cd ${cur_dir}/test_data/llama2_tensor_dump/npu/
    # python3 replace_tensor_dump_data_dir.py
    cd ${cur_dir}/test_data/
     msprobe -f pytorch graph -i ${cur_dir}/test_data/tensor.json -o ${testcase_result_dir} >> ${testcase_result_dir}/plog.txt
    end_time=$(date "+%s")
    ...
    python3 ${cur_dir}/src/validate_visualization_compare_graph_tensor.py ${vis_file} >> ${testcase_result_dir}/plog.txt
