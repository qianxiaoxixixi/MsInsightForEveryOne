...
main() {
    ...
    vis_files=$(find "${testcase_result_dir}" -maxdepth 1 -type f -name 'compare*.vis')
    vis_count=$(echo "$vis_files" | grep -c '^')

    if [ "$vis_count" -ne 4 ]; then
        echo "[error]: The number of vis files is not 4"
        echo "${file_name} fail ${duration_time} ${pytorch_branchs}"
        exit 1
    fi

    vis_file=$(find "${testcase_result_dir}" -maxdepth 1 -type f -name 'compare_step1_rank1*.vis' -print -quit)
