...
main() {
    ...
    msprobe_dump_path="resnet18_tensor_mix"
    msprobe_dump_log="msprobe_dump_log_tensor.log"
    if [ -e ${msprobe_dump_path} ]; then
        rm -rf ${msprobe_dump_path}
        rm -rf ${msprobe_dump_log}
    fi
    python resnet18_tensor.py -mt tensor -mdp ${msprobe_dump_path} --gpu 1 --epochs 1 -b 1 --dummy > ${msprobe_dump_log} 2>&1 &
    ...
    while true; do
        if grep -q "msprobe ends successfully." ${msprobe_dump_log}; then
            ...
            msprobe -f pytorch graph -i ${cur_dir}/test_data/resnet18_tensor_mix.json -o ${testcase_result_dir} >> ${testcase_result_dir}/plog.txt
            end_time=$(date "+%s")
            ...
            python3 ${cur_dir}/src/validate_visualization_resnet18_dump2compare_tensor.py ${vis_file} >> ${testcase_result_dir}/plog.txt
