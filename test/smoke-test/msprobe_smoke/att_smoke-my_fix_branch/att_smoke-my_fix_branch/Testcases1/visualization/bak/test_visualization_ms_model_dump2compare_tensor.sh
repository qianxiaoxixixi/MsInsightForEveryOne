...
main() {
    ...
    start_time=$(date "+%s")
    cd ${cur_dir}/test_data/ms_model
    rm -f ${cur_dir}/test_data/ms_model/msrun_log/worker_0.log

    bash start_msrun_model.sh &
    ...
    while true; do
        if grep -q "msprobe ends successfully." ${cur_dir}/test_data/ms_model/msrun_log/worker_0.log; then
            echo "[info]: Find 'msprobe ends successfully.' in log."
            echo "[info]: Killing mindspore python script train_model.py"
            pkill -f train_model.py

            msprobe -f mindspore graph -i ${cur_dir}/test_data/ms_model/compare.json -o ${testcase_result_dir} >> ${testcase_result_dir}/plog.txt
            end_time=$(date "+%s")
            ...
            python3 ${cur_dir}/src/validate_visualization_ms_model_dump2compare_tensor.py ${vis_file} >> ${testcase_result_dir}/plog.txt
