...
main() {
    ...
    msprobe -f pytorch acc_check -api_info ${api_info_part} -o ${testcase_result_dir} >> ${testcase_result_dir}/plog.txt
    echo "first acc_check finished" >> ${testcase_result_dir}/plog.txt
    msprobe -f pytorch acc_check -api_info ${api_info} -o ${testcase_result_dir} -csv_path ${testcase_result_dir}/accuracy_checking_result_*.csv >> ${testcase_result_dir}/plog.txt
    end_time=$(date "+%s")
