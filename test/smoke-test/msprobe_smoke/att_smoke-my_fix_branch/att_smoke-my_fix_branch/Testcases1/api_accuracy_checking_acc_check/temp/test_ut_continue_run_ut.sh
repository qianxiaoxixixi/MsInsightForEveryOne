...
main() {
    ...
    cd ${att_ut_path}/
    python3 run_ut.py -api_info ${api_info_part} -o ${testcase_result_dir}
    if [ $? -ne 0 ];then
        handle_error "[ERROR] run_ut.py failed during operation."
    fi
    python3 run_ut.py -api_info ${api_info} -o ${testcase_result_dir} -csv_path ${testcase_result_dir}/accuracy_checking_result_*.csv
    if [ $? -ne 0 ];then
        handle_error "[ERROR] run_ut.py continue run ut failed during operation."
    fi
    end_time=$(date "+%s")
