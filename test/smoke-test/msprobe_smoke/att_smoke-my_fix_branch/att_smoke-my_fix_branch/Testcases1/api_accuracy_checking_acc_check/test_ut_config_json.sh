...
main() {
    ...
    msprobe -f pytorch acc_check -api_info ${api_info} -o ${testcase_result_dir} -j -config /home/maoyanlong/att_smoke_new/Testcases1/api_accuracy_checking/json/config.json -f >> ${testcase_result_dir}/plog.txt
    end_time=$(date "+%s")
