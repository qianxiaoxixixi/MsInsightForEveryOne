...
main() {
    ...
    npu_path=${cur_dir}/test_data/test_dm_compare_tensor/ms_data/step0/rank/dump.json
    bench_path=${cur_dir}/test_data/test_dm_compare_tensor/pt_data/step0/rank/dump.json
    stack_path=${cur_dir}/test_data/test_dm_compare_tensor/ms_data/step0/rank/stack.json
    ...
    dm_yaml_path=${cur_dir}/test_data/test_dm_compare_tensor/dm.yaml

    msprobe -f mindspore compare -i ${input_path} -o ${out_dump_path} -s -dm ${dm_yaml_path} >> ${testcase_result_dir}/plog.txt
    end_time=$(date "+%s")
   ...
   echo ${testcase_result_dir}/plog.txt
   ...
   if [ 0 -eq $? ]; then
    ...
    echo ${testcase_result_dir}/out/compare_result_*.csv

    md5_value=$(md5sum ${testcase_result_dir}/out/compare_result_*.csv | awk '{ print $1 }')
    md5_baseline=$(md5sum ${cur_dir}/baseline/test_dm_compare_tensor/out/compare_result_*.csv | awk '{ print $1 }')
