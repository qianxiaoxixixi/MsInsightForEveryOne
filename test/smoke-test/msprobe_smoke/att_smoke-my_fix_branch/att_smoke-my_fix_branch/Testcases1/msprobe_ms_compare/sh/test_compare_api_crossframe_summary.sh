...
main() {
    ...
    npu_path=${cur_dir}/test_data/test_compare_api_crossframe_summary/npu/step0/rank/dump.json
    bench_path=${cur_dir}/test_data/test_compare_api_crossframe_summary/cpu/step0/rank/dump.json
    stack_path=${cur_dir}/test_data/test_compare_api_crossframe_summary/npu/step0/rank/stack.json
    ...
    msprobe -f mindspore compare -i ${input_path} -o ${out_dump_path} >> ${testcase_result_dir}/plog.txt -am ${user_mapping}
    end_time=$(date "+%s")
    ...
    md5_baseline=$(md5sum ${cur_dir}/baseline/test_compare_api_crossframe_summary/out/advisor_*.txt | awk '{ print $1 }')
    ...
    md5_baseline=$(md5sum ${cur_dir}/baseline/test_compare_api_crossframe_summary/out/compare_result_*.csv | awk '{ print $1 }')
    if [ "$md5_value" != "$md5_baseline" ]; then
