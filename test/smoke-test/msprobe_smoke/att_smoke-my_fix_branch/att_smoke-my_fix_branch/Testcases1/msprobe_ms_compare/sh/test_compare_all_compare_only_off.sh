...
main() {
    ...
    md5_baseline=$(md5sum ${cur_dir}/baseline/test_compare_all_compare_only_off/out/advisor_*.txt | awk '{ print $1 }')
    ...
    md5_baseline=$(md5sum ${cur_dir}/baseline/test_compare_all_compare_only_off/out/compare_result_*.csv | awk '{ print $1 }')
    if [ "$md5_value" != "$md5_baseline" ]; then
