...
main() {
    ...
    npu_path=${cur_dir}/test_data/ms_graph_npy_compare_data/npu/
    bench_path=${cur_dir}/test_data/ms_graph_npy_compare_data/bench/
    ...
    if [[ $(ls ${out_dump_path} | wc -w) != 1 ]] || [[ ${result##*.} != "csv" ]]; then
    ...
    md5_baseline=$(md5sum ${cur_dir}/baseline/test_compare.ms_graph_compare_npy/out/compare_result_0_0_*.csv | awk '{ print $1 }')
