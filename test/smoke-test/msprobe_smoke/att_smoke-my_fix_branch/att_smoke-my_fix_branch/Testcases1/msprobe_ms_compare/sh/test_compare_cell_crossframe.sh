...
main() {
    ...
    npu_path=${cur_dir}/test_data/test_compare_cell_cross_frame/L0_tensor_all_cross_framework_ms/step0/rank/dump.json
    bench_path=${cur_dir}/test_data/test_compare_cell_cross_frame/L0_tensor_all_cross_framework_pt/step0/rank/dump.json
    stack_path=${cur_dir}/test_data/test_compare_cell_cross_frame/L0_tensor_all_cross_framework_ms/step0/rank/stack.json
    
    cell_mapping_path=${cur_dir}/cell_mapping.yaml
    ...
    msprobe -f mindspore compare -i ${input_path} -o ${out_dump_path} -cm ${cell_mapping_path} -hl >> ${testcase_result_dir}/plog.txt
    end_time=$(date "+%s")
    ...
    md5_baseline=$(md5sum ${cur_dir}/baseline/test_compare_cell_crossframe/out/advisor_*.txt | awk '{ print $1 }')
    ...
    md5_baseline=$(md5sum ${cur_dir}/baseline/test_compare_cell_crossframe/out/compare_result_*.csv | awk '{ print $1 }')
    if [ "$md5_value" != "$md5_baseline" ]; then
