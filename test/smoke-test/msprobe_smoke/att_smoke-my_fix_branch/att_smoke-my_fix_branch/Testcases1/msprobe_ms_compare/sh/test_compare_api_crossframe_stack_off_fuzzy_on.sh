...
main() {
    ...
    msprobe -f mindspore compare -i ${input_path} -o ${out_dump_path} -f >> ${testcase_result_dir}/plog.txt -am ${user_mapping}
    end_time=$(date "+%s")
    ...
    md5_baseline=$(md5sum ${cur_dir}/baseline/test_compare_api_crossframe_stack_off_fuzzy_on/out/advisor_*.txt | awk '{ print $1 }')
    ...
    md5_baseline=$(md5sum ${cur_dir}/baseline/test_compare_api_crossframe_stack_off_fuzzy_on/out/compare_result_*.csv | awk '{ print $1 }')
    if [ "$md5_value" != "$md5_baseline" ]; then
