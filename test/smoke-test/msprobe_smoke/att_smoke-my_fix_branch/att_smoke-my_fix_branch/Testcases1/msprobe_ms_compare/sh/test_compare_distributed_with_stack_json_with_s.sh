...
main() {
    ...
    npu_path=${cur_dir}/test_data/test_compare_distributed_with_stack_json/step0
    bench_path=${cur_dir}/test_data/test_compare_distributed_with_stack_json/step0
    ...
    msprobe -f mindspore compare -i ${input_path} -o ${out_dump_path} -s >> ${testcase_result_dir}/plog.txt
    end_time=$(date "+%s")
    ...
    md5_baseline=$(md5sum ${cur_dir}/baseline/test_compare_distributed_with_stack_json_with_s/out/compare_result_rank0_*.csv | awk '{ print $1 }')
