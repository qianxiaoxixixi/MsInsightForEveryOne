...
main() {
    ...
    npu_path=${cur_dir}/test_data/test_lm_compare_statistics/ms/dump.json
    bench_path=${cur_dir}/test_data/test_lm_compare_statistics/pt/dump.json
    stack_path=${cur_dir}/test_data/test_lm_compare_statistics/ms/stack.json
    ...
    lm_yaml_path=${cur_dir}/test_data/test_lm_compare_statistics/layer_mapping.yaml

    msprobe -f mindspore compare -i ${input_path} -o ${out_dump_path} -s -lm ${lm_yaml_path} -hl >> ${testcase_result_dir}/plog.txt
    end_time=$(date "+%s")
    ...
    md5_baseline=$(md5sum ${cur_dir}/baseline/test_lm_compare_statistics/out/compare_result_*.csv | awk '{ print $1 }')
