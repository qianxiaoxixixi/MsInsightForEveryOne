...
main() {
    ...
    input_dir_path=${cur_dir}/test_data/test_merge_result_statistics_some_compare_index
    ...
    msprobe -f mindspore merge_result -i ${input_dir_path} -o ${output_dir_path} -config ${config_yaml_path}>> ${testcase_result_dir}/plog.txt
    end_time=$(date "+%s")
    ...
    baseline_xlsx_path=${cur_dir}/baseline/test_merge_result_statistics_some_compare_index/out/multi_ranks_compare_merge_*.xlsx
