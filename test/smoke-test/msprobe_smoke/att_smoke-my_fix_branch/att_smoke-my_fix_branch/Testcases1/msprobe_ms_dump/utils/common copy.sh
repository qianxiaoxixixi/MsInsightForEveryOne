...
check_memory_and_time_variation() {
    ...
    local baseline_log="${baseline_dir}/woker_${rank}.log"
    local output_log="${output_dir}/woker_${rank}.log"

    # 提取 baseline 和 output 中的显存峰值和耗时
    local baseline_mem=$(grep -oP "显存峰值\s+\K[\d.]+(?=GB)" "${baseline_log}" | sort -nr | head -n 1)
    local output_mem=$(grep -oP "显存峰值\s+\K[\d.]+(?=GB)" "${output_log}" | sort -nr | head -n 1)
