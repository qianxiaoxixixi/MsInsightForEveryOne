...
context.set_context( device_id=3,deterministic='ON')
...
# 定义一个 Callback 来调用 PrecisionDebugger
class DebuggerCallback(Callback):
    def __init__(self, debugger, steps):
    ...
    def begin(self, run_context):
        ...
        self.debugger.start(cb_params.network)
...
if __name__ == "__main__":
    ...
    # 初始化 PrecisionDebugger 和 Callback
    script_dir = os.path.dirname(os.path.abspath(__file__))
    config_path = os.path.join(script_dir, 'config.json')
    debugger = PrecisionDebugger(config_path=config_path)
    debugger_callback = DebuggerCallback(debugger, steps=5)

    # 使用 model.train 进行训练
    model.train(epoch=1, train_dataset=[(data, label)], callbacks=[debugger_callback], dataset_sink_mode=False)
