...
main() {
    ...
    sed -i "s#\"dump_data_dir\".*#\"dump_data_dir\":\"${cur_dir}/json/${file_name}/tensor_data\",#" $api_info

    cd ${att_ut_path}/
    python3 run_overflow_check.py -api_info ${api_info} >> ${testcase_result_dir}/plog.txt
    end_time=$(date "+%s")
