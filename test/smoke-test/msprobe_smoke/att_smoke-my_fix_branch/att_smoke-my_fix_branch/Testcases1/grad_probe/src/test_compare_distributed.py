...
def main(rank, world_size, free_npu_info, gm):
...
if __name__ == "__main__":
    ...
    if n_device >= 2:
        world_size = n_device
        mp.spawn(main, args=(world_size, free_npu_info, debugger), nprocs=world_size, join=True)
    else:
        device = 0
        main(0, -1, free_npu_info, debugger)
    
    from msprobe import *
    GradComparator.compare_distributed(debugger.gm._output_path,
                                    debugger.gm._output_path,
                                    debugger.gm._output_path)
