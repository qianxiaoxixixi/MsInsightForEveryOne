...
config_filepath=${testcase_result_dir}/config.json
...
main() {
    ...
    cmd="python ${src_dir}/test_compare_distributed.py ${config_filepath} ${n_procs} >> ${testcase_result_dir}/plog.txt"
    echo $cmd
    ...
    similarity_file=(`find ${rank_dirs[0]} -type f -name 'similarities*'`)
    similarity_lines=(`wc -l ${similarity_file}`)
    if [ ${similarity_lines} -ne `expr ${weight_grad_num} + 2` ]; then
        handle_error "similarity_file number wrong"
    fi

    similarity_picture_dir=(`find ${rank_dirs[0]} -type d -name 'similarities*'`)
    similarity_picture_files=(`find ${similarity_picture_dir} -type f -name '*png'`)
    similarity_picture_files_num=${#similarity_picture_files[@]}
    if [ ${similarity_picture_files_num} -ne `expr ${weight_grad_num} + 1` ]; then
        handle_error "similarity_picture_files number wrong"
    fi
