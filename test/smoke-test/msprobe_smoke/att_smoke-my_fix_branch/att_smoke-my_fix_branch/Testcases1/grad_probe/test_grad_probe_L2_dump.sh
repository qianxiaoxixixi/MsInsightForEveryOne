...
header="param_name,\"(-inf, -1]\",\"(-1, 0]\",\"(0, 1]\",\"(1, inf)\",=0,max,min,norm,shape"
header="param_name,\"(-inf, -1]\",\"(-1, 0]\",\"(0, 1]\",\"(1, inf)\",=0,max,min,norm,shape"
...
main() {
    ...
    cmd="python ${src_dir}/test_L2_dump.py ${config_filepath} ${n_procs} >> ${testcase_result_dir}/plog.txt"
    echo $cmd
    ...
    count=$(echo "$result_header" | grep -o "," | wc -l)
    if [ ${count} -ne 13 ]; then
        echo ${result_header}
