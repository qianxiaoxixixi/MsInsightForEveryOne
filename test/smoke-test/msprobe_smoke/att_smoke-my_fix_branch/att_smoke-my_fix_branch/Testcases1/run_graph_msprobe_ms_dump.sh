...
# 找出 JSON 文件和层级文件名的交集
json_files=$(ls ${testcases_dir}/graph_msprobe_ms_dump/json | grep '\.json$') #一级目录
# json_files=$(find ${testcases_dir}/graph_msprobe_ms_dump/json -type f -name '*.json' | xargs -n 1 basename) #目录递归查找
...
if [[ -z "${common_files}" ]]; then
    ...
    echo "[Info][$(date "+%F %T")] ---------------------------- Begin Run graph_msprobe_ms_dump TestCase ----------------------------"
    for casename in ${common_files}; do
        echo "[Info][$(date "+%F %T")] ---------------------------- Execution ${casename} ----------------------------"
        #bash "${testcases_dir}/graph_msprobe_ms_dump/test_dump.sh" "${casename}"
	    bash "${testcases_dir}/graph_msprobe_ms_dump/test_dump.sh" "${casename}" >> "${result_log}"
    done
