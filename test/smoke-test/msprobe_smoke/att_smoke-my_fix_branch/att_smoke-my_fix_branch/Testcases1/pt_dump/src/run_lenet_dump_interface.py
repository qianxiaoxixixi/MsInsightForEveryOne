...
def train_one_epoch(data_loader, model, optimizer, criterion):
    ...
    if "start_model_interface" in config_json:
        for image, label in data_loader:
            ...
            debugger.stop()
            debugger.step()
    elif "double_start_end_interface" in config_json:
        for image, label in data_loader:
            debugger.start()
            ...
            debugger.step()
    elif "module_dump_interface" in config_json:
        for image, label in data_loader:
            debugger.start()

            image, label = image.to(device), label.to(device)
            optimizer.zero_grad()
            module_dump(model, "my_model")
            output = model(image)
            loss = criterion(output, label)
            loss.backward()
            module_dump_end()
