...
main() {
    ...
    npu_path=${cur_dir}/test_data/test_compare_md5_mode/npu/step0/rank/dump.json
    bench_path=${cur_dir}/test_data/test_compare_md5_mode/cpu/step0/rank/dump.json
    stack_path=${cur_dir}/test_data/test_compare_md5_mode/npu/step0/rank/stack.json
    ...
    input_result_path=${out_compare_path}/compare_result_*.xlsx
    ...
    count=$(wc -l ${out_compare_path}/compare_result_*.csv | awk -F " " '{print $1}')
    result=$(ls $out_compare_path)
    if [[ $(ls ${out_compare_path} | wc -w) != 3 ]]; then
    ...
    md5_baseline=$(md5sum ${cur_dir}/baseline/test_compare_md5/out/advisor_*.txt | awk '{ print $1 }')
    ...
    md5_baseline=$(md5sum ${cur_dir}/baseline/test_compare_md5/out/compare_result_*.csv | awk '{ print $1 }')
    if [ "$md5_value" != "$md5_baseline" ]; then
