...
main() {
    ...
    npu_path=${cur_dir}/test_data/test_compare_exception/dump_json_shape/dump.json
    bench_path=${cur_dir}/test_data/test_compare_exception/dump_json_shape/dump.json
    stack_path=${cur_dir}/test_data/test_compare_exception/dump_json_shape/stack.json
    ...
    grep "not list or tuple" ${testcase_result_dir}/plog.txt
    if [ $? -ne 0 ]; then
        echo "[shape object type] Check Error in log file." >> ${testcase_result_dir}/run_log.txt
