...
main() {
    ...
    input_dir_path=${cur_dir}/test_data/test_merge_result_exception_diff_dump_mode
    output_dir_path=${testcase_result_dir}/out
    config_yaml_path=${cur_dir}/yaml/config_statistics_some_indexes.yaml
    ...
    if [[ $(ls ${output_dir_path} | wc -w) != 1 ]]; then
        echo "[ERROR] The result file is not generated." >> ${testcase_result_dir}/run_log.txt
        echo "${file_name} fail ${duration_time} ${pytorch_branchs}"
        exit 1
    fi

    grep "Compare indexes in rank1 compare result are not consistent with those in other compare results" ${testcase_result_dir}/plog.txt && grep "Only one compare result gets merge data" ${testcase_result_dir}/plog.txt
    if [ $? -ne 0 ]; then
        echo "[ERROR] diff dump task check error in log file." >> ${testcase_result_dir}/run_log.txt
        echo "${file_name} fail ${duration_time} ${pytorch_branchs}"
        exit 1
    fi

    baseline_xlsx_path=${cur_dir}/baseline/test_merge_result_exception_diff_dump_mode/out/multi_ranks_compare_merge_*.xlsx
