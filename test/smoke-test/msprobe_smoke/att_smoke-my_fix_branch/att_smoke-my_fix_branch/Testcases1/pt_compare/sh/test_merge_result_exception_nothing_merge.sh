...
main() {
    ...
    input_dir_path=${cur_dir}/test_data/test_merge_result_statistics_nothing_merge
    ...
    grep "Nothing to merge" ${testcase_result_dir}/plog.txt
    if [ $? -ne 0 ]; then
        echo "[ERROR] Nothing to merge check error in log file." >> ${testcase_result_dir}/run_log.txt
        echo "${file_name} fail ${duration_time} ${pytorch_branchs}"
        exit 1
    fi
