...
main() {
    ...
    npu_path=${cur_dir}/test_data/test_compare_summary_mode_llama2_7b/npu/step0/rank0/dump.json
    bench_path=${cur_dir}/test_data/test_compare_summary_mode_llama2_7b/cpu/step0/rank0/dump.json
    stack_path=${cur_dir}/test_data/test_compare_summary_mode_llama2_7b/npu/step0/rank0/stack.json
    ...
    msprobe -f pytorch compare -i ${input_path} -o ${out_dump_path} -hl >> ${testcase_result_dir}/plog.txt
    end_time=$(date "+%s")
    ...
    md5_baseline=$(md5sum ${cur_dir}/baseline/test_compare_summary_default_llama2_7b/out/advisor_*.txt | awk '{ print $1 }')
    ...
    md5_baseline=$(md5sum ${cur_dir}/baseline/test_compare_summary_default_llama2_7b/out/compare_result_*.csv | awk '{ print $1 }')
    if [ "$md5_value" != "$md5_baseline" ]; then
