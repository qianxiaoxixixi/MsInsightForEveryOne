...
main() {
    ...
    input_dir_path=${cur_dir}/test_data/test_merge_result_exception_one_empty_xlsx
    ...
    grep "Rank1 compare result is empty and will not shown in merged result" ${testcase_result_dir}/plog.txt && grep "Only one compare result get merge data" ${testcase_result_dir}/plog.txt
    if [ $? -ne 0 ]; then
        echo "[ERROR] only one compare result check error in log file." >> ${testcase_result_dir}/run_log.txt
        echo "${file_name} fail ${duration_time} ${pytorch_branchs}"
        exit 1
    fi

    baseline_xlsx_path=${cur_dir}/baseline/test_merge_result_exception_one_empty_xlsx/out/multi_ranks_compare_merge_*.xlsx
