...
main() {
    ...
    npu_path=${cur_dir}/test_data/test_compare_all_mode/npu/step0/rank/dump.json
    bench_path=${cur_dir}/test_data/test_compare_all_mode/cpu/step0/rank/dump.json
    stack_path=${cur_dir}/test_data/test_compare_all_mode/npu/step0/rank/stack.json
    ...
    msprobe -f pytorch compare -i ${input_path} -o ${out_dump_path} -c >> ${testcase_result_dir}/plog.txt
    end_time=$(date "+%s")
    ...
    if [[ $(ls ${out_dump_path} | wc -w) != 2 ]]; then
    ...
    md5_baseline=$(md5sum ${cur_dir}/baseline/test_compare_all_compare_only_on/out/compare_result_*.csv | awk '{ print $1 }')
