...
main() {
    ...
    config_yaml_path=${cur_dir}/yaml/config_empty.yaml
    ...
    grep "config.yaml is empty, please check" ${testcase_result_dir}/plog.txt
    if [ $? -ne 0 ]; then
        echo "[ERROR] config.yaml empty check error in log file." >> ${testcase_result_dir}/run_log.txt
