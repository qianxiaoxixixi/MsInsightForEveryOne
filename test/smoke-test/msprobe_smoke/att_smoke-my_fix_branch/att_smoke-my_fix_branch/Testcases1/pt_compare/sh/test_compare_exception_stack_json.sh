...
main() {
    ...
    npu_path=${cur_dir}/test_data/test_compare_exception/stack_json/dump.json
    bench_path=${cur_dir}/test_data/test_compare_exception/stack_json/dump.json
    stack_path=${cur_dir}/test_data/test_compare_exception/stack_json/stack.json
    ...
    msprobe -f pytorch compare -i ${input_path} -o ${out_dump_path} -s >> ${testcase_result_dir}/plog.txt
    end_time=$(date "+%s")
    duration_time=$(( ${end_time} - ${start_time} ))

    grep "special characters" ${testcase_result_dir}/plog.txt
    if [ $? -ne 0 ]; then
        echo "[special characters] Check Error in log file." >> ${testcase_result_dir}/run_log.txt
