...
# test_ms_continue_run_ut

file_name=$(basename $0 .sh)
log_dir=${output_dir}/ms_api_accuracy_checker_log
log_file=${log_dir}/${file_name}_log.txt
output_path=${output_dir}/ms_api_accuracy_checker_output/test_ms_continue_run_ut
...
# run
# msprobe -f mindspore multi_run_ut -api_info ${dump_json_path} -o ${output_path} -csv_path ${csv_path} -d 0 1 2 3 >> ${log_file}
msprobe -f mindspore run_ut -api_info ${dump_json_path} -o ${output_path} -csv_path ${output_path}/accuracy_checking_result_20241016152330.csv  >> ${log_file}
