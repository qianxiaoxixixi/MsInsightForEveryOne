...
def compare_dynative_output(base_path, real_path):
    ...
    for base_file in base_files:
        ...
        if len(base_file)>= 4 and base_file[-4:] == ".npy":
            ...
            if np.isnan(value1).any() and np.isnan(value2).any():
                continue
            if 'Distributed._reduce_scatter_base' in real_file and 'forward.input.0' in real_file:
                continue
            if not (value1 == value2).all():
                print(f"npy: {real_file.replace('pass', '').replace('fail', '')} has different value")
                return False
        elif len(base_file) >= 5 and base_file[-5:] == ".json":
            if "stack.json" in base_file:
                base_size = os.path.getsize(base_file)
                real_size = os.path.getsize(base_file)
                if base_size > 500 and real_size > 500:
                    if base_size > real_size * 1.2 or real_size > base_size * 1.2:
                        print("stack.json is different")
                        return False
            else:
                if not filecmp.cmp(base_file, real_file):
                    print(f"other json: {real_file.replace('pass', '').replace('fail', '')} is different")
                    return False
...
if __name__ == '__main__':
    if len(sys.argv) == 3:
        if compare_dynative_output(sys.argv[1], sys.argv[2]):
            print("pass")
        else:
            print("fail")
    else:
