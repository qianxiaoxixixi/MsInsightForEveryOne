...
main() {
    ...
    python gen_data.py post ${cur_dir}/test_data/test_normal/step0 ${cur_dir}/test_data/test_post_anomalies/step0
    msprobe -f pytorch nan_analyze -i ${cur_dir}/test_data/test_post_anomalies/step0 -o ${testcase_result_dir} >> ${testcase_result_dir}/plog.txt

    end_time=$(date "+%s")
    duration_time=$(( ${end_time} - ${start_time} ))
    rm -rf ${cur_dir}/test_data/test_post_anomalies
    ...
    python3 ${cur_dir}/src/validate.py post ${res_file} >> ${testcase_result_dir}/plog.txt
