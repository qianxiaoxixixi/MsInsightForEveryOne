...
for file in ${common_files}; do
    if [[ $file == test_large* ]] || [[ $file == *moe* ]]; then
        Larger_files+=("$file")
    else
...
main() {
    ...
    if [[ "$level_to_run" -eq 4 ]]; then
    	...
    	for task in ${tasks[@]}; do
        	...
        	bash ${testcases_dir}/pt_dump/test_dump_compare.sh ${output_dir} ${task} ${pytorch_branchs} >> ${result_log}
    	done
    fi
