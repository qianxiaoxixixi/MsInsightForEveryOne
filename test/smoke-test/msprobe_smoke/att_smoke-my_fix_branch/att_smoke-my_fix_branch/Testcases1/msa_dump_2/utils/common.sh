...
handle_error() {
    ...
    echo "${file_name} fail ${duration_time}s msadapter"
    exit 1
}
...
compare_similarity() {
    ...
    ((lines_file1 = lines_file1 + 1))
    lines_file2=$(wc -l < "$file2")
    ((lines_file2 = lines_file2 + 1))
    ...
    if (( $(echo "$percent_diff > $allow_diff" | bc -l) )); then
        return 1
    fi
    return 0
}
...
compare_dump() {
    ...
    if [ "$input_count" -ne "$baseline_count" ]; then  
        handle_error "[ERROR] The number of files in input_dir ($input_dir) is different from that in baseline_dir ($baseline_dir)." 
    fi

    # 当level设置为L2时，只判断文件数量。
    if [[ "$input_dir" == *"L2"* ]]; then
    ...
    find "$baseline_dir" -type f | while read -r file1; do  
        ...
        if [[ -f "$file2" ]]; then
            ...
            if [[ "$(basename "$file1")" == "stack.json" ]]; then
                compare_similarity $file1 $file2 19
                if [ $? -eq 1 ];then
                    handle_error "Failed to check the $file1 and $file2 files."
                fi
                continue  
            ...
            if [[ "$(basename "$file1")" == "dump.json" ]]; then
                compare_similarity $file1 $file2 0
                if [ $? -eq 1 ];then
                    handle_error "Failed to check the $file1 and $file2 files."
                fi
            ...
            handle_error "[ERROR] The $relative_path file does not exist in input_dir $input_dir."  
        fi
    done
