...
install_ptdbg_tools() {
    ...
    export ADAPTER_TARGET_PYTHON_PATH=/home/maoyanlong/miniconda3/envs/${conda_env}/bin/python3
    bash ./configure
    mkdir build
    cd build
    cmake ..
...
download_install_personal_msfmktransplt() {
    ...
    if [ -d ${download_path} ]; then
        local tools_path="/usr/local/Ascend/ascend-toolkit/latest/tools/ms_fmk_transplt"
