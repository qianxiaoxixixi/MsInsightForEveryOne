...
# 定义锁文件路径
LOCKFILE="/tmp/atat_smoke_jenkins.lock"
...
# 尝试获取锁（将会阻塞，直到获取到锁）
{
    ...
    update_branch() {
        ...
        # git download
        if echo $url | grep -q "https://gitcode.com/Ascend"; then
        ...
        elif echo $url | grep -q "head"; then
            # pull/1127/head:pr_1127
            echo "install tools accroding fetch method"
            git clone -b ${branch} https://gitcode.com/Ascend/mstt.git ${download_path}

            cd ${download_path}
            # git fetch https://gitee.com/ascend/mstt.git $url
	    # git fetch https://gitee.com/ascend/msit.git pull/3898/head:pr_3898
	    git fetch https://gitcode.com/Ascend/mstt.git +refs/merge-requests/$url
    ...
    main() {
	    ...
	    elif [ "msprobe_mt_dump" == "${modules}" ]; then
	    ...
	    #bash ${testcases}/run_ms_set_dump.sh 2
            #bash ${testcases}/run_ms_acl_dump.sh 3
 	    #bash ${testcases}/run_ms_38BV3.sh 4 ${branch}
        ...
        elif [ "ms_acl_dump" == "${modules}" ]; then
            bash ${testcases}/run_ms_acl_dump.sh 3
	    elif [ "nan_analyze" == "${modules}" ]; then
            bash ${testcases}/run_nan_analyze.sh
        elif [ "ms_38BV3" == "${modules}" ]; then
            bash ${testcases}/run_ms_38BV3.sh 4 ${branch}
	elif [ "msa_dump_2" == "${modules}" ]; then
