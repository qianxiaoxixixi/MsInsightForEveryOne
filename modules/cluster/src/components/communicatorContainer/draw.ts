...
const drawHorizontalLine = (svg: d3.Selection<d3.BaseType, unknown, HTMLElement, any>): void => {
    dpContinerSize.width = (dpContinerSize.padding * 2) +
    ...
    let start = ppContinerSize.left + (rankSize.width / 2) + 2;
    let end = start + dpContinerSize.width + 4;
    const top = ppContinerSize.height - 5;
    let path = `${start},${top} ${start},${top - 47}`;
    ...
    for (let i = 0; i < parallelSize.dpSize; i++) {
        ...
        start = end;
        end = start + dpContinerSize.width + 4;
        path = `${start},${top} ${start},${top - 47}`;
    }
    path = `${ppContinerSize.left + (rankSize.width / 2) + 2},${top} ${start - dpContinerSize.width - 4},${top}`;
    g.append('polyline')
...
const drawVerticalLine = (svg: d3.Selection<d3.BaseType, unknown, HTMLElement, any>): void => {
    svg.attr('height', `${ppContinerSize.top + (ppContinerSize.height * parallelSize.ppSize) + 20}px`);
    if (parallelSize.ppSize < 2) {
        return;
    }
    let start = ppContinerSize.top + rankSize.height + 2;
    let end = start + ppContinerSize.height - 34;
    const left = ppContinerSize.left + (rankSize.width / 2) + 12;
    ...
    for (let i = 1; i < parallelSize.ppSize; i++) {
        g.append('polyline')
            ...
            .style('pointer-events', 'visibleStroke')
            .style('cursor', 'pointer');
        start = end + (rankSize.height - rankSize.padding);
        end = start + ppContinerSize.height - (rankSize.height - rankSize.padding);
...
const drawDpLine = (svg: d3.Selection<d3.BaseType, unknown, HTMLElement, any>): void => {
    ...
    let startX = ppContinerSize.left + rankSize.width - 15;
    const startY = ppContinerSize.top + 40;
...
const transformLine = (svg: d3.Selection<d3.BaseType, unknown, HTMLElement, any>,
    ...
    const verticalLineChangeX = (site[1] * (dpContinerSize.width + 4)) + (site[3] * rankSize.width);
    ...
    const dpLineChangeX = (site[1] * (dpContinerSize.width + 4)) + (site[2] * tpContinerSize.width);
...
const drawDpRect = (svg: d3.Selection<d3.BaseType, unknown, HTMLElement, any>): void => {
    ...
    let x = 16;
    const y = 4;
    const width = dpContinerSize.width - 1;
    const height = (ppContinerSize.height * parallelSize.ppSize) - 2;
    for (let i = 0; i < parallelSize.dpSize; i++) {
        g.append('rect')
            .attr('x', x)
            .attr('y', y)
...
const drawPpRect = (svg: d3.Selection<d3.BaseType, unknown, HTMLElement, any>): void => {
    ...
    let x = 65;
    const y = 20;
    const width = rankSize.width - 10;
    const height = (ppContinerSize.height * parallelSize.ppSize) - 40;
    for (let i = 0; i < parallelSize.dpSize; i++) {
...
const drawTpRect = (svg: d3.Selection<d3.BaseType, unknown, HTMLElement, any>): void => {
    ...
    const width = rankSize.width * parallelSize.preTpCount;
    const height = rankSize.height + 10;
