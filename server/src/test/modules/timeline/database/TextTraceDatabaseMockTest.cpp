...
/**
 * text场景测试CommitData，未初始化场景，插入失败
 */
TEST_F(TextTraceDatabaseMockTest, TestCommitDataWhenNotInitThenInsertFailed)
{
    std::recursive_mutex sqlMutex;
    MockDatabase database(sqlMutex);
    sqlite3 *dbPtr = nullptr;
    ...
    std::tuple<int64_t, std::string, std::string> thread = { trackId, "ppp", "qqqq" };
...
/**
 * text场景测试CommitData，初始化了，插入成功
 */
TEST_F(TextTraceDatabaseMockTest, TestCommitDataWhenInitThenInsertSuccess)
{
    std::recursive_mutex sqlMutex;
    MockDatabase database(sqlMutex);
    sqlite3 *dbPtr = nullptr;
    ...
    DatabaseTestCaseMockUtil::CreateTable(dbPtr, threadTableSql);
    const uint64_t trackId = 1;
    const std::string tid = "hhh";
    const std::string pid = "qqq";
    std::tuple<int64_t, std::string, std::string> thread = { trackId, tid, pid };
    database.InitStmt();
    database.AddThreadCache(thread);
    database.CommitData();
    std::vector<ThreadPO> threadPOS;
    ...
    EXPECT_EQ(threadPOS[0].tid, tid);
    EXPECT_EQ(threadPOS[0].pid, pid);
}

/**
 * text场景测试CommitData，初始化了，但表不存在，插入失败
 */
TEST_F(TextTraceDatabaseMockTest, TestCommitDataWhenInitAndTableNotExistThenInsertFailed)
{
    std::recursive_mutex sqlMutex;
    MockDatabase database(sqlMutex);
    sqlite3 *dbPtr = nullptr;
