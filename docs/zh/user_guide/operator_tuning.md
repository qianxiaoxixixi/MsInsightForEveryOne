...
<tbody>
  <tr>
    <td>trace.json</td>
    <td>算子仿真指令流水图文件。</td>
    <td>参见《<a href="https://gitcode.com/Ascend/msopprof/blob/master/docs/zh/msopprof_simulator_user_guide.md">msopprof simulator用户指南</a>》。</td>
    <td>时间线（Timeline）</td>
  </tr>
  <tr>
    <td rowspan="3">visualize_data.bin</td>
    <td>指令流水图可视化呈现文件。</td>
    <td>参见《<a href="https://gitcode.com/Ascend/msopprof/blob/master/docs/zh/msopprof_user_guide.md">msopprof用户指南</a>》和《<a href="https://gitcode.com/Ascend/msopprof/blob/master/docs/zh/msopprof_simulator_user_guide.md">msopprof simulator用户指南</a>》。</td>
    <td>时间线（Timeline）</td>
  </tr>
  <tr>
    <td>算子基础信息、计算单元负载和Roofline瓶颈分析等信息的可视化呈现文件。</td>
    <td>参见《<a href="https://gitcode.com/Ascend/msopprof/blob/master/docs/zh/msopprof_user_guide.md">msopprof用户指南</a>》。</td>
    <td>详情（Details）</td>
  </tr>
  <tr>
...
- 仅昇腾910\_95 AI处理器支持通过“msopprof”方式采集指令流水图可视化呈现文件，数据展示在时间线（Timeline）界面。
...
![](./figures/operator_tuning/时间线界面.png "时间线界面")
...
- 单击界面左上方工具栏中的![](./figures/operator_tuning/zh-cn_image_0000002500200380.png)，在弹出输入框中输入需要搜索的指令，然后按回车键，则会匹配对应的指令，搜索结果匹配指令总数，如[**图 1**  搜索指令总数](#搜索指令总数)所示，搜索到与名称中包含“mov”相关的指令总数为11754。
    ...
    **图 1**  搜索指令总数<a id="搜索指令总数"></a>  
    ![](./figures/operator_tuning/搜索指令总数.png "搜索指令总数")
...
- 单击界面左上方工具栏中的![](./figures/operator_tuning/zh-cn_image_0000002500200380.png)，可在搜索弹出输入框左侧分别单击![](./figures/operator_tuning/zh-cn_image_0000002500200386.png)和![](./figures/operator_tuning/zh-cn_image_0000002532040315.png)，开启大小写匹配和全词匹配功能，如[**图 2**  大小写匹配和全词匹配](#大小写匹配和全词匹配)所示。
    ...
    **图 2**  大小写匹配和全词匹配<a id="大小写匹配和全词匹配"></a>  
    ![](./figures/operator_tuning/大小写匹配和全词匹配.png "大小写匹配和全词匹配")

- 单击搜索框后方的切换按钮，可以查看上一个或者下一个匹配的指令，也可以在输入框后方输入具体的数字搜索其对应的指令，该指令将会被选中并显示在界面的中间，如[**图 3**  定位指令](#定位指令)所示。

    **图 3**  定位指令<a id="定位指令"></a>  
    ![](./figures/operator_tuning/定位指令.png "定位指令")
...
- 单击搜索框后方的“在查找窗口打开”，可跳转至界面下方的“查找”页签，展示所有搜索指令列表，如[**图 4**  在查找窗口打开](#在查找窗口打开-2)所示，字段解释如[**表 2**  字段说明](#字段说明)所示。单击“点击跳转Timeline”列的“点击”可跳转到指令在时间线视图上的具体位置。
    ...
    **图 4**  在查找窗口打开<a id="在查找窗口打开-2"></a>  
    ![](./figures/operator_tuning/在查找窗口打开-2.png "在查找窗口打开-2")
...
- 单点标记
  ...
  **图 3**  单点标记<a id="单点标记-6"></a>  
  ![](./figures/operator_tuning/单点标记-6.png "单点标记-6")
...
- 标记管理
  ...
  **图 4**  查看标记信息<a id="查看标记信息-7"></a>  
  ![](./figures/operator_tuning/查看标记信息-7.png "查看标记信息-7")
...
- MindStudio Insight支持指令间同步连线关系（SET\_FLAG到WAIT\_FLAG）展示，单击有连线的指令，即可显示该指令关联的连线，即使折叠连线起点或者终点的Pipe，连线也不会消失，如[**图 5**  指令间连线](#指令间连线)所示。
    ...
    **图 5**  指令间连线<a id="指令间连线"></a>  
    ![](./figures/operator_tuning/指令间连线.png "指令间连线")
...
- MindStudio Insight支持全量连线的功能，单击界面左上方工具栏中的![](./figures/operator_tuning/zh-cn_image_0000002531920295.png)，在弹框中选择一个或多个连线类型，则在时间线视图中展示对应Pipe间的所有连线，如[**图 6**  全量连线](#全量连线-8)所示。
    ...
    **图 6**  全量连线<a id="全量连线-8"></a>  
    ![](./figures/operator_tuning/全量连线-8.png "全量连线-8")
...
- 支持隐藏SET\_FLAG和WAIT\_FLAG指令。
    ...
    **图 7**  隐藏SET/WAIT事件<a id="隐藏SET-WAIT事件"></a>  
    ![](./figures/operator_tuning/隐藏SET-WAIT事件.png "隐藏SET-WAIT事件")
    ...
    **图 8**  显示SET/WAIT事件<a id="显示SET-WAIT事件"></a>  
    ![](./figures/operator_tuning/显示SET-WAIT事件.png "显示SET-WAIT事件")
...
- 使用鼠标左键可在单个Pipe级泳道，或跨多个core泳道框选部分指令，框选部分区域指令后，可在下方选中列表显示指令的统计信息，如[**图 1**  选中列表](#选中列表-9)所示，字段解释如[**表 1**  选中列表字段说明](#选中列表字段说明)。
    ...
    **图 1**  选中列表<a id="选中列表-9"></a>  
    ![](./figures/operator_tuning/选中列表-9.png "选中列表-9")
...
- 当选中单个指令时，可在下方选中详情显示该指令的详情信息，如[**图 2**  选中详情](#选中详情-10)所示，字段解释如[**表 2**  选中详情字段说明](#选中详情字段说明)所示。
    ...
    **图 2**  选中详情<a id="选中详情-10"></a>  
    ![](./figures/operator_tuning/选中详情-10.png "选中详情-10")
...
![](./figures/operator_tuning/源码界面.png "源码界面")
...
![](./figures/operator_tuning/搜索源码.png "搜索源码")
...
![](./figures/operator_tuning/查看关联指令.png "查看关联指令")

勾选指令表上的“仅查看关联指令”则会进行过滤，只显示此行代码关联的指令，如[**图 3**  过滤关联指令](#过滤关联指令)所示。

**图 3**  过滤关联指令<a id="过滤关联指令"></a>  
![](./figures/operator_tuning/过滤关联指令.png "过滤关联指令")
...
![](./figures/operator_tuning/查看关联代码.png "查看关联代码")
...
![](./figures/operator_tuning/筛选指令表.png "筛选指令表")
...
![](./figures/operator_tuning/详情界面.png "详情界面")
...
![](./figures/operator_tuning/核间负载分析.png "核间负载分析")

> [!NOTE] 说明  
> 
> - <term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>、<term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>和<term>昇腾910\_95 AI处理器</term>导出的性能数据支持此模块。
...
>[!NOTE] 说明
> 
> - 仅<term>昇腾910\_95 AI处理器</term>、<term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>、<term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>和<term>Atlas 推理系列产品</term>支持此模块。
> - 当导入<term>昇腾910\_95 AI处理器</term>的数据时，Roofline模型图上将展示指令类型，可以结合图中参数进行筛选，以便查看Roofline模型图。

- 当硬件产品为<term>昇腾910\_95 AI处理器</term>、<term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>、<term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>时，Roofline性能模型分析包含内存单元、内存通路以及搬运单元页签。

    **内存单元**：显示HBM/L2和Memory Unit模型图，如[**图 3**  内存单元](#内存单元)所示，参数解释如[**表 4**  内存单元参数说明](#内存单元参数说明)所示。

    **图 3**  内存单元<a id="内存单元"></a>  
    ![](./figures/operator_tuning/内存单元.png "内存单元")
    ...
    **图 4**  内存通路<a id="内存通路"></a>  
    ![](./figures/operator_tuning/内存通路.png "内存通路")
    ...
    **图 5**  搬运单元<a id="搬运单元"></a>   
    ![](./figures/operator_tuning/搬运单元.png "搬运单元")
...
- 当硬件产品为<term>Atlas 推理系列产品</term>时，仅存在内存单元内容，如[**图 6**  内存单元模型图](#内存单元模型图)所示，参数解释如[**表 7**  内存单元参数说明](#内存单元参数说明-1)所示。
    ...
    **图 6**  内存单元模型图<a id="内存单元模型图"></a>  
    ![](./figures/operator_tuning/内存单元模型图.png "内存单元模型图")
...
![](./figures/operator_tuning/计算工作负载分析.png "计算工作负载分析")
...
![](./figures/operator_tuning/内存负载分析.png "内存负载分析")
...
![](./figures/operator_tuning/查看Cycles.png "查看Cycles")
...
![](./figures/operator_tuning/显示算子性能信息.png "显示算子性能信息")
...
- 计算工作负载分析（Compute Workload Analysis）：以柱状图和数据窗格呈现方式查看相应信息，柱状图中蓝色为对比数据，绿色为基线数据，数据窗格显示算子间的对比差异，单击“详情”列中“查看更多”，可展示基线数据和对比数据的详情，如[**图 3**  计算工作负载分析对比](#计算工作负载分析对比)所示。
    ...
    **图 3**  计算工作负载分析对比<a id="计算工作负载分析对比"></a>  
    ![](./figures/operator_tuning/计算工作负载分析对比.png "计算工作负载分析对比")
...
- 内存负载分析（Memory Workload Analysis）：以内存热力图和数据窗格呈现方式查看相应信息，热力图中括号里的数据为基线数据，括号外的数据为对比数据，数据窗格显示算子间的对比差异，单击“详情”列中“查看更多”，可展示基线数据和对比数据的详情，如[**图 4**  内存负载分析对比](#内存负载分析对比)所示。
    ...
    **图 4**  内存负载分析对比<a id="内存负载分析对比"></a>  
    ![](./figures/operator_tuning/内存负载分析对比.png "内存负载分析对比")
...
![](./figures/operator_tuning/缓存界面.png "缓存界面")
...
![](./figures/operator_tuning/跳转指令表.png "跳转指令表")
