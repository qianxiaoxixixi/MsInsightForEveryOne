...
>[!NOTE]  
> 这份数据使用了并行策略，因此每隔一段时间需要通过“通信”同步各个卡的数据。
> 通信时间长表示卡在等待其他卡做好准备进行数据同步，通信时间短表示卡还在做其他操作，未准备好进行数据同步。
...
>[!NOTE]  
>Overlap Analysis 泳道中 Computing 子泳道是上方 Ascend Hardware 泳道的投影，表示卡正在计算；Communication 子泳道是上方 Communication 泳道的投影，表示卡正在通信；Free 子泳道表示卡正在空闲
