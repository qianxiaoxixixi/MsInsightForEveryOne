...
![](./figures/system_tuning/时间线（Timeline）界面.png "时间线（Timeline）界面")
...
<tbody>
  ...
  <tr>
    ...
    <td class="tg-0pky">-</td>
    <td class="tg-0pky">低功耗数据，包含功耗、带宽、频率、温度等数据，通过呈现变频曲线，准确识别算子执行过程中的变频情况。<br>Low Power泳道仅支持展示<term>昇腾910_95 AI处理器</term>导出的性能数据。</td>
  </tr>
  <tr>
    <td class="tg-0pky">Biu Perf</td>
    <td class="tg-0pky">Group&lt;id&gt;-aiv&lt;id&gt;</td>
    <td class="tg-0pky">呈现SU、VEC、CUBE、MTE等指令执行时间，以及打点数据。<br>Biu Perf泳道仅支持展示<term>昇腾910_95 AI处理器</term>导出的性能数据。</td>
  </tr>
  <tr>
    <td class="tg-0pky">UB</td>
    <td class="tg-0pky">UDMA/UNIC-Ports&lt;id&gt;</td>
    <td class="tg-0pky">为UDMA和UNIC两种数据类型，呈现UB总体收发带宽情况。<br>UB泳道仅支持展示<term>昇腾910_95 AI处理器</term>导出的性能数据。</td>
  </tr>
  <tr>
    <td class="tg-0pky" rowspan="2">Block Detail</td>
    <td class="tg-0pky">AIC/AIV Earliest</td>
    <td class="tg-0pky">展示各个算子在最早的AI core或AI Vector Core上的执行时间，当算子为Mix类型时，会同时执行在AIC和AIV上。<br>Block Detail泳道仅支持展示<term>昇腾910_95 AI处理器</term>导出的性能数据。</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td class="tg-0pky">包含集合通信指令数据，CCU任务的起止时间以及CCU任务的一级索引指令的起止时间，以及同步及数据搬运耗时。<br>CCU泳道仅支持展示<term>昇腾910_95 AI处理器</term>导出的性能数据。</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td class="tg-0pky">DVPP和DSA加速器读带宽。<br>acc_pmu泳道在<term>昇腾910_95 AI处理器</term>不支持该泳道。</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td class="tg-0pky">数据流通道的接收、发送带宽。SIO泳道仅支持在text格式文件下展示。<br>SIO泳道仅支持展示<term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>和<term>昇腾910_95 AI处理器</term>DIE间传输带宽信息。</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td class="tg-0pky">PA流量接收等级。当有集合通信带宽时，不建议参考该字段值，该字段为粗粒度的统计值。Stars Chip Trans泳道仅支持在text格式文件下展示，且<term>昇腾910_95 AI处理器</term>不支持该泳道。</td>
  </tr>
  <tr>
...
- 单击界面左上方工具栏中的![](./figures/system_tuning/zh-cn_image_0000002500040294.png)，在弹出输入框中输入需要搜索的内容，然后按回车键，则会匹配对应的算子或API，搜索结果匹配算子和API总数，在界面中也会高亮显示匹配的算子或API，如[**图 1**  搜索算子](#搜索算子)所示，搜索到与名称为“npu”相关的算子和API总数为3104。
    ...
    **图 1**  搜索算子<a id="搜索算子"></a>  
    ![](./figures/system_tuning/搜索算子.png "搜索算子")
...
- 单击界面左上方工具栏中的![](./figures/system_tuning/zh-cn_image_0000002500040294.png)，可在搜索弹出输入框左侧分别单击![](./figures/system_tuning/zh-cn_image_0000002531920185.png)和![](./figures/system_tuning/zh-cn_image_0000002500040296.png)，开启大小写匹配和全词匹配功能。
    - 单击![](./figures/system_tuning/zh-cn_image_0000002531920185.png)开启大小写匹配，在弹出输入框中输入需要搜索的内容，然后按回车键，则会匹配名称中包含搜索项的算子或API，如[**图 2**  开启大小写匹配或全词匹配](#开启大小写匹配或全词匹配)所示。
        ...
        **图 2**  开启大小写匹配或全词匹配<a id="开启大小写匹配或全词匹配"></a>   
        ![](./figures/system_tuning/开启大小写匹配或全词匹配.png "开启大小写匹配或全词匹配")
...
- 单击搜索框后方的“在查找窗口打开”，可跳转至页面下方的“查找”页签，展示搜索项相关信息，如[**图 3**  在查找窗口打开](#在查找窗口打开)所示，字段解释如[**表 2**  查找页签字段说明](#查找页签字段说明)所示。单击“点击跳转Timeline”列的“点击”可跳转到算子或API在时间线视图上的具体位置。
    ...
    **图 3**  在查找窗口打开<a id="在查找窗口打开"></a>   
    ![](./figures/system_tuning/在查找窗口打开.png "在查找窗口打开")
...
- 在不展开进程级泳道的情况下，根据线程级中时间轴上的数据，将以灰色填充进程级泳道来展示线程级泳道中的数据分布情况，如[**图 1**  时间线（Timeline）界面预览](#时间线（Timeline）界面预览)中的2所示。
    ...
    **图 1**  时间线（Timeline）界面预览<a id="时间线（Timeline）界面预览"></a>  
    ![](./figures/system_tuning/时间线（Timeline）界面预览.png "时间线（Timeline）界面预览")
...
![](./figures/system_tuning/集群场景时间线数据展示.png "集群场景时间线数据展示")
...
![](./figures/system_tuning/定位文件夹.png "定位文件夹")
...
- 按卡显示：以只显示1卡为例，单击界面左上方![](./figures/system_tuning/zh-cn_image_0000002532040213.png)，选择“卡过滤”，然后单击输入框，在下拉框选择“1”，即可显示1卡的时间线（Timeline）信息，如[**图 4**  卡过滤](#卡过滤)所示。
    ...
    **图 4**  卡过滤<a id="卡过滤"></a>   
    ![](./figures/system_tuning/卡过滤.png "卡过滤")
...
- 按泳道显示：以只显示每张卡的Overlap Analysis泳道为例，单击界面左上方工具栏![](./figures/system_tuning/zh-cn_image_0000002532040213.png)，选择“泳道过滤”，然后单击输入框，在下拉框选择“Overlap Analysis”，即可显示Overlap Analysis泳道的时间线（Timeline）信息，如[**图 5**  泳道过滤](#泳道过滤)所示。
    ...
    **图 5**  泳道过滤<a id="泳道过滤"></a>  
    ![](./figures/system_tuning/泳道过滤.png "泳道过滤")
...
- MindStudio Insight支持固定并置顶泳道，且可以通过鼠标拖拽对收起状态的置顶泳道进行自由排序，方便同其他同类层级进行对比。
    ...
    **图 6**  置顶对比<a id="置顶对比"></a>  
    ![](./figures/system_tuning/置顶对比.png "置顶对比")
...
- MindStudio Insight还支持一键置顶同一通信域的通信泳道。
    ...
    **图 7**  置顶通信泳道<a id="置顶通信泳道"></a>  
    ![](./figures/system_tuning/置顶通信泳道.png "置顶通信泳道")
    ...
    **图 8**  取消置顶<a id="取消置顶"></a>  
    ![](./figures/system_tuning/取消置顶.png "取消置顶")
...
- 手动设置对齐到起始位置
    ...
    **图 9**  初始位置偏移量<a id="初始位置偏移量"></a>  
    ![](./figures/system_tuning/初始位置偏移量.png "初始位置偏移量")
...
- 手动设置偏移量
    ...
    **图 10**  单卡时间调整<a id="单卡时间调整"></a>  
    ![](./figures/system_tuning/单卡时间调整.png "单卡时间调整")
    ...
    **图 11**  泳道时间调整<a id="泳道时间调整"></a>  
    ![](./figures/system_tuning/泳道时间调整.png "泳道时间调整")
...
![](./figures/system_tuning/多机多卡展示.png "多机多卡展示")
...
- 单点标记
    ...
    **图 15**  单点标记<a id="单点标记"></a>  
    ![](./figures/system_tuning/单点标记.png "单点标记")
...
- 标记管理
    ...
    **图 16**  查看标记信息<a id="查看标记信息"></a>  
    ![](./figures/system_tuning/查看标记信息.png "查看标记信息")
...
- MindStudio Insight支持算子连线关系展示，单击有连线的算子，即可显示该算子关联的连线，即使折叠连线起点或者终点的进程，连线也不会消失。并且支持定位到该算子关联连线的其它算子，并显示其详情。
    ...
    2. 单击鼠标右键，选择“跳转到连线算子”，在算子列表中单击“来源”或“目标”下的具体算子名称或连线方式，即可跳转至对应算子，并自动高亮显示该算子，同时在“选中详情”页签中显示该算子的详细信息。如[**图 17**  算子连线关系](#算子连线关系)所示。
        ...
        **图 17**  算子连线关系<a id="算子连线关系"></a>  
        ![](./figures/system_tuning/算子连线关系.png "算子连线关系")
...
- MindStudio Insight支持全量连线的功能，单击界面左上方工具栏中的![](./figures/system_tuning/zh-cn_image_0000002500200298.png)，在弹框中选择一个或多个连线类型，也可在搜索框中搜索某一连线类型的关键字，勾选相应的连线类型，则在图形化窗格展示对应类型的所有连线，如[**图 18**  全量连线](#全量连线)所示。
    ...
    **图 18**  全量连线<a id="全量连线"></a>  
    ![](./figures/system_tuning/全量连线.png "全量连线")
...
- 一键全部解析：在时间线（Timeline）界面，单击“开始全局解析”，将开始解析所有卡的数据，如[**图 19**  全局解析](#全局解析)所示。当所有卡的数据解析完成后，“开始全局解析”按钮消失。
    ...
    **图 19**  全局解析<a id="全局解析"></a>  
    ![](./figures/system_tuning/全局解析.png "全局解析")
...
- 部分解析：当只需要解析部分卡的数据时，可逐个单击对应卡序号后面的![](./figures/system_tuning/zh-cn_image_0000002500040320.png)，解析所选卡的数据，如[**图 20**  单卡解析](#单卡解析)所示。当对应卡数据解析完成后，按钮消失，如图中0卡和1卡所示。
    ...
    **图 20**  单卡解析<a id="单卡解析"></a>  
    ![](./figures/system_tuning/单卡解析.png "单卡解析")
    ...
    **图 21**  过滤展示并解析<a id="过滤展示并解析"></a>  
    ![](./figures/system_tuning/过滤展示并解析.png "过滤展示并解析")
...
- 相同通信域的卡解析：当解析完一个卡后，在通信的Group子泳道上，单击鼠标右键，选择“解析相关通信域的卡”，和该泳道通信域相关的卡都被解析，如[**图 22**  解析相关通信域的卡](#解析相关通信域的卡)，解析完成后，该鼠标右键菜单变为“已解析全部相关通信域的卡”并置灰。
    ...
    **图 22**  解析相关通信域的卡<a id="解析相关通信域的卡"></a>  
    ![](./figures/system_tuning/解析相关通信域的卡.png "解析相关通信域的卡")
...
- **取消基准算子**
    ...
    **图 26** 取消基准算子<a id="取消基准算子"></a>
    ![](./figures/system_tuning/取消基准算子.png "取消基准算子")
...
- **恢复默认偏移量**
    ...
    **图 27** 恢复所有卡的默认偏移量<a id="恢复所有卡的默认偏移量"></a>
    ![](./figures/system_tuning/恢复所有卡的默认偏移量.png "恢复所有卡的默认偏移量")
...
- 隐藏泳道
    ...
    **图 1** 隐藏泳道<a id="隐藏泳道"></a><br>
    ![](./figures/system_tuning/隐藏泳道.png "隐藏泳道")
...
- 显示全部已隐藏的泳道
    ...
    **图 2** 显示隐藏的泳道<a id="显示隐藏的泳道"></a><br>
    ![](./figures/system_tuning/显示隐藏的泳道.png "显示隐藏的泳道")
...
- 隐藏Python调用栈：选择需要隐藏Python调用栈的泳道，单击鼠标右键，在弹出菜单中选择“隐藏Python调用栈”，隐藏该泳道下Python调用栈内容，如[**图 3** 隐藏Python调用栈](#隐藏Python调用栈)所示。
    ...
    **图 3** 隐藏Python调用栈<a id="隐藏Python调用栈"></a><br>
    ![](./figures/system_tuning/隐藏Python调用栈.png "隐藏Python调用栈")
...
- 显示Python调用栈：在已经隐藏了Python调用栈信息的泳道，单击鼠标右键，在弹出菜单中选择“显示Python调用栈”，显示该泳道下被隐藏的Python调用栈内容，如[**图 4** 显示Python调用栈](#显示Python调用栈)所示。
    ...
    **图 4** 显示Python调用栈<a id="显示Python调用栈"></a><br>
    ![](./figures/system_tuning/显示Python调用栈.png "显示Python调用栈")
...
- 开启泳道高度自适应：在展开的泳道上，单击鼠标右键，在弹出菜单中选择“开启泳道高度自适应”，可自动调整泳道高度，以适配当前页面显示，如[**图 7** 开启泳道高度自适应](#开启泳道高度自适应)所示。
    ...
    **图 7** 开启泳道高度自适应<a id="开启泳道高度自适应"></a><br>
    ![](./figures/system_tuning/开启泳道高度自适应.png "开启泳道高度自适应")
...
- 关闭泳道高度自适应：在已开启泳道高度自适应功能的泳道上，单击鼠标右键，在弹出菜单中选择“关闭泳道高度自适应”，关闭泳道高度自适应功能，泳道恢复初始高度，如[**图 8** 关闭泳道高度自适应](#关闭泳道高度自适应)所示。
    ...
    **图 8** 关闭泳道高度自适应<a id="关闭泳道高度自适应"></a><br>
    ![](./figures/system_tuning/关闭泳道高度自适应.png "关闭泳道高度自适应")
...
- 锁定框选区域
    ...
    **图 9** 锁定框选区域<a id="锁定框选区域"></a>
    ![](./figures/system_tuning/锁定框选区域.png "锁定框选区域")
...
- 解锁框选区域
    ...
    **图 10** 解锁框选区域<a id="解锁框选区域"></a>
    ![](./figures/system_tuning/解锁框选区域.png "解锁框选区域")
...
- 合并泳道
    ...
    **图 11** 合并泳道<a id="合并泳道"></a><br>
    ![](./figures/system_tuning/合并泳道.png "合并泳道")
...
- 取消合并
    ...
    **图 12** 取消合并泳道<a id="取消合并泳道"></a><br>
    ![](./figures/system_tuning/取消合并泳道.png "取消合并泳道")
...
- 使用鼠标左键在单个三层级泳道上框选部分区域的算子，或在单卡层级下跨多个泳道框选部分算子，框选部分区域算子后，可在下方“选中列表”页签中显示算子的统计信息，如[**图 1** 选中列表](#选中列表)所示，字段解释如[**表 1** 选中列表字段说明](#选中列表字段说明)所示。
    ...
    **图 1** 选中列表<a id="选中列表"></a>
    ![](./figures/system_tuning/选中列表.png "选中列表")
    ...
    **表 1** 选中列表字段说明<a id="选中列表字段说明"></a>
    ...
    3. 单击鼠标右键，取消勾选“算子框选模式”，或在数据窗格中标题行右侧，取消勾选“算子框选模式”，即可关闭按照深度框选算子开关。
        ...
        **图 2** 算子框选模式<a id="算子框选模式"></a>
        ![](./figures/system_tuning/算子框选模式.png "算子框选模式")
...
- 当选中单个算子时，可在下方“选中详情”页签中显示该算子的详情信息，如[**图 3** 选中详情](#选中详情)所示，字段解释如[**表 2** 选中详情字段说明](#选中详情字段说明)所示。
    ...
    **图 3** 选中详情<a id="选中详情"></a>
    ![](./figures/system_tuning/选中详情.png "选中详情")
...
- 统计系统视图：展示综合指标信息、API类型信息以及算子详情等信息，具体请参见[统计系统视图](#统计系统视图)。
...
2. 单击鼠标右键，分别可按需选择菜单，进行分析，如[**表 3** 时间范围分析](#时间范围分析)和[**图 4** 时间范围分析](#时间范围分析-1)所示。
    ...
    **图 4** 时间范围分析<a id="时间范围分析-1"></a>
    ![](./figures/system_tuning/时间范围分析.png "时间范围分析")

3. 开启该功能后，可在泳道上单击鼠标右键，选择“移除时间范围分析”，可关闭该功能。

**统计系统视图**<a id="统计系统视图"></a>

在“系统视图”页签，当选择“统计系统视图”时，页面包含卡序号（Rank ID）选框、综合指标（Overall Metrics）、5种类型的算子汇总统计页签和算子详情（Kernel Details）（NPU上算子的详细信息），在卡序号选框中可以选择想要查看的卡。如果是db场景，需要依次选择“机器名称”和“卡序号”。

综合指标（Overall Metrics）展示所有算子的总体信息，如[**图 5** 综合指标](#综合指标)所示，字段解释如[**表 4** 综合指标字段说明](#综合指标字段说明)所示，当选择计算时间（Computing Time）列表中的子层级时，可单击“More”区域任一算子，会跳转到该算子在时间线视图中的具体位置。

**图 5** 综合指标<a id="综合指标"></a>
![](/figures/system_tuning/综合指标.png "综合指标")
...
|更多|More|当选择Computing Time（计算时间）列表中的子层级时，该区域展示所选层级的所有算子详情，可单击任意一个算子，跳转至时间线视图中算子所在的具体位置。|

5种算子类型包括Python API 汇总（Python API Summary）、CANN API 汇总（CANN API Summary）、Ascend HardWare Task 汇总（Ascend HardWare Task Summary）、通信汇总（Communication Summary）、覆盖分析（Overlap Analysis），算子信息如[**图 6** 算子汇总页签](#算子汇总页签)所示，字段解释如[**表 5** 统计系统视图字段说明](#统计系统视图字段说明)所示。

**图 6** 算子汇总页签<a id="算子汇总页签"></a>
![](/figures/system_tuning/算子汇总页签.png "算子汇总页签")

**表 5** 统计系统视图字段说明<a id="统计系统视图字段说明"></a>
...
算子详情（Kernel Details）展示NPU上算子的详细信息，如[**图 7** 算子详情信息展示](#算子详情信息展示)所示，字段解释如[**表 6** 算子详情字段说明](#算子详情字段说明)所示，单击“点击跳转Timeline”列中的“点击”，会跳转到算子在时间线视图中的具体位置，区域四（数据窗格）将会展示选中详情，展示该算子的具体信息。单击算子详情表中字段名称后的![](/figures/system_tuning/zh-cn_image_0000002531920231.png)，可对相关字段进行模糊搜索。

**图 7** 算子详情信息展示<a id="算子详情信息展示"></a>
![](/figures/system_tuning/算子详情信息展示.png "算子详情信息展示")

**表 6** 算子详情字段说明<a id="算子详情字段说明"></a>
...
6种专家建议系统包括亲和 API（Affinity API）、亲和优化器（Affinity Optimizer）、AICPU 算子（AICPU Operators）、ACLNN 算子（ACLNN Operators）、算子融合（Operators Fusion）和算子下发（Operators Dispatch），如[**图 8** 专家系统视图](#专家系统视图)所示，字段解释如[**表 7** 专家系统视图字段说明](#专家系统视图字段说明)所示。

选择任一专家建议系统，右侧区域会显示该类专家建议系统的详细信息，单击“点击跳转Timeline”列中的“点击”，会跳转到算子在时间线视图中的具体位置，区域四（数据窗格）“选中详情”页签将会展示该算子的具体信息。

**图 8** 专家系统视图<a id="专家系统视图"></a>
![](/figures/system_tuning/专家系统视图.png "专家系统视图")

**表 7** 专家系统视图字段说明<a id="专家系统视图字段说明"></a>
...
在时间线（Timeline）界面，选择所需泳道，单击鼠标右键，单击“在事件视图中显示”菜单，跳转至“系统视图”页签，左侧区域选项默认选择“事件视图”，右侧区域显示该泳道所有算子详情，如[**图 9** 事件视图](#事件视图)所示，字段解释如[**表 8** 事件视图字段说明](#事件视图字段说明)所示。

**图 9** 事件视图<a id="事件视图"></a>
![](/figures/system_tuning/事件视图.png "事件视图")

**表 8** 事件视图字段说明<a id="事件视图字段说明"></a>
...
![](./figures/system_tuning/动态图内存界面.png "动态图内存界面")
...
![](./figures/system_tuning/静态图内存界面.png "静态图内存界面")
...
![](./figures/system_tuning/切换卡序号.png "切换卡序号")
...
![](./figures/system_tuning/切换维度.png "切换维度")
...
![](./figures/system_tuning/选中放大区域.png "选中放大区域")
...
- 动态图场景
    ...
    **图 4** 搜索算子<a id="搜索算子-1"></a>
    ![](./figures/system_tuning/搜索算子-1.png "搜索算子-1")
...
- 静态图场景
    ...
    **图 5** 静态图搜索算子<a id="静态图搜索算子"></a>
    ![](./figures/system_tuning/静态图搜索算子.png "静态图搜索算子")
...
- 当“分组方式”选择“组件”时，不支持算子搜索功能。界面如[**图 6** 组件级内存申请/释放详情](#组件级内存申请/释放详情)所示，字段解释如[**表 3** 字段说明](#字段说明)所示。
    ...
    **图 6** 组件级内存申请/释放详情<a id="组件级内存申请/释放详情"></a>
    ![](./figures/system_tuning/组件级内存申请-释放详情.png "组件级内存申请-释放详情")
...
![](./figures/system_tuning/算子高亮显示.png "算子高亮显示")
...
![](./figures/system_tuning/性能对比.png "性能对比")
...
![](./figures/system_tuning/算子界面.png "算子界面")
...
![](./figures/system_tuning/计算算子.png "计算算子")
...
![](./figures/system_tuning/计算算子类型.png "计算算子类型")
...
![](./figures/system_tuning/计算算子名称和输入shape.png "计算算子名称和输入shape")
...
![](./figures/system_tuning/通信算子.png "通信算子")
...
![](./figures/system_tuning/通信算子类型.png "通信算子类型")
...
![](./figures/system_tuning/算子对比.png "算子对比")
...
![](./figures/system_tuning/概览界面.png "概览界面")
...
- 并行策略概览包括并行策略设置、并行策略图展示和专家建议，如[**图 2** 并行策略概览](#并行策略概览)所示，并行策略参数解释如[**表 1** 并行策略参数说明](#并行策略参数说明)所示。当设置好并行策略后，可分别选择“DP + PP”、“DP + PP + CP”、“DP + PP + TP”或“DP + PP + CP + TP”并行维度展示并行策略图，选择图中目标编号，可悬浮显示该目标编号的详情信息，进行查看；也可单击鼠标右键弹出菜单，单击“复制属性”，复制当前目标编号的详情信息，粘贴至本地进行查看分析。
    ...
    **图 2** 并行策略概览<a id="并行策略概览"></a>
    ![](./figures/system_tuning/并行策略概览.png "并行策略概览")
...
- 计算/通信概览（Computation/Communication Overview），计算及通信概览，柱状图展示计算或通信算子的迭代耗时数据，折线图展示计算或通信算子的耗时占比数据，专家建议（Advice）是对计算通信域内各卡的计算时间、通信时间（未被覆盖）和空闲时间进行数据分析后给出的建议，帮助开发者快速分析，如[**图 3** 计算/通信概览](#计算/通信概览)所示，参数解释请参见[**表 2** 计算/通信概览参数说明](#计算/通信概览参数说明)所示。
    ...
    **图 3** 计算/通信概览<a id="计算/通信概览"></a>
    ![](./figures/system_tuning/计算-通信概览.png "计算-通信概览")
...
- 计算详情（Rank  _ID_）（Computing Detail（Rank  _ID_）），当选择“DP + PP + CP + TP”和“DP + PP + TP”并行维度时，单击计算/通信概览区域中某个节点的柱状图时，将展示该节点加速核的总耗时和利用率，单击对应的“详情”后展示计算算子的详细信息，如[**图 4** 计算算子详细信息](#计算算子详细信息)所示，字段解释如[**表 3** 计算详情字段说明](#计算详情字段说明)所示。还可单击表格右上角![](./figures/system_tuning/zh-cn_image_0000002500040346.png)按钮，一键复制表中当前所展示的内容，并粘贴至Excel表格中进行分析。
    ...
    **图 4** 计算算子详细信息<a id="计算算子详细信息"></a>
    ![](./figures/system_tuning/计算算子详细信息.png "计算算子详细信息")
...
- 通信详情（Rank  _ID_）（Communication Detail（Rank  _ID_）），当选择“DP + PP + CP + TP”和“DP + PP + TP”并行维度时，单击计算/通信概览区域中某个节点的柱状图时，将展示该节点通信算子的总耗时（包含未被覆盖的通信时长和被覆盖的通信时长），单击对应的“详情”后展示通信算子的详细信息，如[**图 5** 通信算子详细信息](#通信算子详细信息)所示，字段解释如[**表 4** 通信详情字段说明](#通信详情字段说明)所示。还可单击表格右上角![](./figures/system_tuning/zh-cn_image_0000002500040346.png)按钮，一键复制表中当前所展示的内容，并粘贴至Excel表格中进行分析。
    ...
    **图 5** 通信算子详细信息<a id="通信算子详细信息"></a>
    ![](./figures/system_tuning/通信算子详细信息.png "通信算子详细信息")
...
- 当选择“DP + PP + CP + TP”和“DP + PP + TP”并行维度时，单击并行策略图中单卡图标，出现连线，单击流水线并行连线，会展示流水并行图，如[**图 6** 流水并行图](#流水并行图)所示。
    ...
    **图 6** 流水并行图<a id="流水并行图"></a>
    ![](./figures/system_tuning/流水并行图.png "流水并行图")
...
![](./figures/system_tuning/MoE大模型专家负载均衡分析.png "MoE大模型专家负载均衡分析")
...
![](./figures/system_tuning/数据并行策略.png "数据并行策略")
...
- 连线联动
    ...
    **图 2** 联动功能<a id="联动功能"></a>
    ![](./figures/system_tuning/联动功能.png "联动功能")
...
- 框选联动
    ...
    **图 3** 框选联动<a id="框选联动"></a>
    ![](./figures/system_tuning/框选联动.png "框选联动")
...
- DP + PP维度
    ...
    **图 4** DP + PP维度<a id="DP-+-PP维度"></a>
    ![](./figures/system_tuning/DP-+-PP维度.png "DP-+-PP维度")
...
- DP + PP + CP维度
    ...
    **图 5** DP + PP + CP维度<a id="DP-+-PP-+-CP维度"></a>
    ![](./figures/system_tuning/DP-+-PP-+-CP维度.png "DP-+-PP-+-CP维度")
...
- DP + PP + TP维度
    ...
    **图 6** DP + PP + TP维度<a id="DP-+-PP-+-TP维度"></a>
    ![](./figures/system_tuning/DP-+-PP-+-TP维度.png "DP-+-PP-+-TP维度")
...
- DP + PP + CP + TP维度
    ...
    **图 7** DP + PP + CP + TP维度<a id="DP-+-PP-+-CP-+-TP维度"></a>
    ![](./figures/system_tuning/DP-+-PP-+-CP-+-TP维度.png "DP-+-PP-+-CP-+-TP维度")
...
![](./figures/system_tuning/概览界面对比模式.png "概览界面对比模式")
...
![](./figures/system_tuning/MoE模型专家负载均衡分析.png "MoE模型专家负载均衡分析")
...
![](./figures/system_tuning/通信矩阵.png "通信矩阵")
...
![](./figures/system_tuning/通信耗时分析.png "通信耗时分析")
...
![](./figures/system_tuning/带宽分析.png "带宽分析")
...
![](./figures/system_tuning/通信算子详情.png "通信算子详情")
...
![](./figures/system_tuning/专家建议.png "专家建议")
...
- 通信（Communication）界面支持通信算子缩略图显示：
    ...
    - 单击通信算子缩略图中的单个算子，支持悬浮显示该算子所属卡序号、算子名称、开始时间和总耗时，如[**图 1** 通信算子缩略图](#通信算子缩略图)所示，图中各字段解释参考[**表 1** 算子悬浮显示字段说明](#算子悬浮显示字段说明)。
        ...
        **图 1** 通信算子缩略图<a id="通信算子缩略图"></a>
        ![](./figures/system_tuning/通信算子缩略图.png "通信算子缩略图")
...
![](./figures/system_tuning/跳转至时间线视图.png "跳转至时间线视图")
...
![](./figures/system_tuning/跳转至通信算子缩略图.png "跳转至通信算子缩略图")
...
![](./figures/system_tuning/算子对齐.png "算子对齐")
...
![](./figures/system_tuning/筛选范围.png "筛选范围")
...
![](./figures/system_tuning/慢卡列表.png "慢卡列表")
...
- 通信矩阵（Communication Matrix）
    ...
    **图 7** 通信矩阵对比模式<a id="通信矩阵对比模式"></a>
    ![](./figures/system_tuning/通信矩阵对比模式.png "通信矩阵对比模式")
...
- 通信耗时分析（Communication Duration Analysis）
    ...
    **图 8** 通信界面对比模式<a id="通信界面对比模式"></a>
    ![](./figures/system_tuning/通信界面对比模式.png "通信界面对比模式")
    ...
    **图 9** 通信时长数据分析对比详情<a id="通信时长数据分析对比详情"></a>
    ![](./figures/system_tuning/通信时长数据分析对比详情.png "通信时长数据分析对比详情")
...
![](./figures/system_tuning/强化学习界面.png "强化学习界面")
...
MindStudio Insight工具提供了一套完整的数据采集和转化脚本，能够采集Linux Kernel ftrace数据，并将其转换为支持的json文件格式；同时，该工具还支持将转换后的Linux Kernel ftrace数据和Profiling的数据同时导入，实现在同一界面展示两种数据，便于对比分析，从而提高Host Bound问题定位的效率。

MindStudio Insight工具支持导入转换后的Linux Kernel ftrace数据json文件和Profiling数据，Profiling数据可导入单卡或集群数据。

### 数据采集

**Linux Kernel ftrace数据采集**

在采集Linux Kernel ftrace数据前，需先安装trace-cmd命令，获取MindStudio Insight提供的采集脚本，具体请参见[**表 1** 采集前任务说明](#采集前任务说明)。

**表 1** 采集前任务说明<a id="采集前任务说明"></a>

|任务|说明|
|--|--|
|安装trace-cmd命令|可执行以下命令，安装trace-cmd命令。<br> - Ubuntu系统：`sudo apt-get install trace-cmd`<br> - CentOS系统：`sudo yum install trace-cmd`|
|获取采集脚本|采集脚本名称为“trace_record.py”。安装MindStudio Insight后，采集脚本在{install-path}/resources/profiler/scripts/ftrace_tools目录下，其中{install-path}是MindStudio Insight的安装路径。|

Linux Kernel ftrace数据有两种采集方式，分别为非侵入式采集和侵入式采集，可根据需求选择合适的采集方式。

- 非侵入式采集

    非侵入式采集方式无需修改训练脚本，可直接使用trace\_record.py脚本，只需输入正确的参数取值即可。

    执行以下命令，采集数据。其中命令中的参数说明请参见[**表 2** 参数说明](#参数说明)。

    ```shell
    python trace_record.py -h --cpu ${cpu mask} --output ${output} --record_time ${record_time}
    ```
  ...
  |--|--|
  |-h，--help|显示帮助信息。|
  |--cpu|cpu_mask列表，多个CPU使用半角逗号分割。例如采集0，1，4，则输入--cpu=0,1,4。|
  |--output|输出的文件名称。|
  |--record_time|采集的时间，单位秒。示例：--record_time=60。如果传递小于等于0的数字，则会持续采集，直到使用Ctrl+C终止进程。|

    采集示例如下。

    1. 使用远程登录工具，在终端中启动训练脚本。其中train.py为训练脚本。

        ```shell
        python train.py
        ```

    2. 在另外的终端中启动采集脚本，采集时间为60s，采集数据。

        ```shell
        python trace_record.py --record_time=60
        ```

- 侵入式采集

    侵入式采集方式可通过采集脚本trace\_record.py中提供的接口，在用户代码对应位置调用接口，进行采集。该方式灵活性高，可对特性的逻辑进行采集。接口信息请参见[**表 3** 接口说明](#接口说明)。

    **表 3** 接口说明<a id="接口说明"></a>

  |接口|说明|
  |--|--|
  |def ftrace_record_start(cpu_list)|开始采集接口，作用是打开Linux Kernel ftrace数据采集开关。其中**cpu_list**参数为CPU采集列表，用于指定需要采集的CPU，默认取值为None，表示采集所有CPU数据。格式采取数组形式，数组内容为CPU编号，例如指定采集CPU1和CPU4，则输入cpu_list=[1, 4]。|
  |def ftrace_record_stop(output)|停止采集并保存数据接口，作用是关闭Linux Kernel ftrace数据采集开关，并将采集到的数据写入指定的output文件中，例如output='/tmp/ftrace.txt'。需要注意的是，保存数据需要一定的时间，会阻塞线程。|

    上述两个接口可在任意代码中添加使用，也可作为一个整体代码使用。

    接口使用示例如下，在已有的Profiling开关开启/关闭处添加采集开始和结束接口，执行采集任务。

    ```shell
    import ftrace_record
    ftrace_record_start(cpu_list=[0, 1, 4])
    profiling_start()
    train()    # 用户代码
    profiling_stop()
...
Profiling数据采集可参见《性能调优工具指南》中的“[msprof采集通用命令](https://www.hiascend.com/document/detail/zh/mindstudio/830/T&ITools/Profiling/atlasprofiling_16_0008.html)”和“[PyTorch训练场景性能分析快速入门](https://www.hiascend.com/document/detail/zh/mindstudio/830/T&ITools/Profiling/atlasprofiling_16_0004.html)”章节内容。

**Linux Kernel ftrace数据处理**

MindStudio Insight工具提供了数据转换脚本trace\_convert.py，可将采集到的Linux Kernel ftrace数据转换为可视化的流水图格式，便于使用MindStudio Insight展示。

数据转换脚本trace\_convert.py在“\{install-path\}/resources/profiler/scripts/ftrace\_tools”目录下，其中\{install-path\}是MindStudio Insight的安装路径。

执行以下命令，转换数据。其中命令中的参数说明请参见[**表 4** 参数说明](#参数说明-1)。

```shell
python trace_convert.py -h --input ${inputpath} --output ${outputpath} --cpu_list ${cpu_list} --profiling_data ${profiling_data_path}
```

**表 4** 参数说明<a id="参数说明-1"></a>

|参数|说明|
|--|--|
|-h，--help|显示帮助信息。|
|--input|Linux Kernel ftrace数据文件路径。|
|--output|转换后的json格式文件存放路径。|
|--cpu_list|筛选指定的CPU，建议在采集时指定所需的cpu_list。|
|--profiling_data|采集到的Profiling数据存放目录，用于将Linux Kernel ftrace数据与Profiling数据时间轴对齐。|

使用示例如下。例如采集的Profiling数据存放目录为“result\_dir/ctl\_1418857\_20251025030529768\_ascend\_pt”，采集到的Linux Kernel ftrace数据文件存放目录为“result\_dir/ftrace.txt”，执行以下命令，进行数据转换。

```shell
python trace_convert.py --input=result_dir/ftrace.txt --profiling_data=result_dir/ctl_1418857_20251025030529768_ascend_pt
```

### 数据分析

1. 打开MindStudio Insight工具，导入采集的Profiling数据。
2. 在导入的Profiling数据工程名称后，单击![](./figures/system_tuning/zh-cn_image_0000002500200468.png)，在当前工程中导入转换后的Linux Kernel ftrace数据json文件，可以看到界面上两种数据并存的时间线流水图。
3. 选择CPU Scheduling子泳道，查看CPU侧调度情况。

    **图 1** 查看CPU Scheduling<a id="查看CPU Scheduling"></a>
    ![](./figures/system_tuning/查看CPU-Scheduling.png "查看CPU-Scheduling")

4. 选择Process Scheduling子泳道，查看特定进程的调度状态。

    **图 2** 查看Process Scheduling<a id="查看Process Scheduling"></a>
    ![](./figures/system_tuning/查看Process-Scheduling.png "查看Process-Scheduling")

5. 可使用MindStudio Insight的置顶功能，将有问题的泳道置顶对比，观察调度情况。
